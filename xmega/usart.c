...
int buffer_pop(Buffer* b, uint8_t* data){
  ...
  *data = b->data[b->start]; //get the first element and increment index
...
void resolve_buffers(int bytes){
  ...
  while((in_buffer.start != in_buffer.end || ((out_buffer.start != (out_buffer.end+1)%MAX_BUFFER_LENGTH) && !in_queue)) && bytes-- > 0){
    ...
    else if((out_buffer.start != (out_buffer.end+1)%MAX_BUFFER_LENGTH) && !in_queue) turn  = IN_QUEUE; //and vice versa
    ...
    //handle errors generated by interrupts
...
void resolve_single_output(){
  ...
  ++out_count;  //incremet this messages sent byte count
...
ISR(USARTC0_DRE_vect){
  ...
  if(status != OK){ //shutdown if buffer is empty
    usart_busy_flag = 0;  //usart is now busy
