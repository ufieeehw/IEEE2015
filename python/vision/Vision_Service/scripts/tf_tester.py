...
def handle_camera_pose():
    translation = (4, 4, 0)#assuming camera is in the slot closest to edge
    print translation
    rotation = tf.transformations.quaternion_from_euler(0,0,2)
    print rotation
    time = rospy.Time.now()
...
def read_spinner_servo():
    ...
    rotation = tf.transformations.quaternion_from_euler(0, 1.570796, 1) #rotates about the y axis to make the z axis along the motor, servo rotation value is sent to z rotation
    time = rospy.Time.now()
...
def read_shoulder_servo():
    ...
    rotation = tf.transformations.quaternion_from_euler(1.570796, 0, 1) #may include a constant shift to follow axis reference conventions, will have a published angle from shoulder servo
    time = rospy.Time.now() 
...
def read_elbow_servo():
    ...
    rotation = tf.transformations.quaternion_from_euler(0, 0, 1) #may include a constant shift to follow axis reference conventions, will have a published angle from elbow servo
    time = rospy.Time.now() 
...
def read_wrist_joint(): #end-effector always parallel to the floor, angle reationship between the wrist joint and elbow reference fram  
    ...
    rotation = tf.transformations.quaternion_from_euler(0, 0, -1) #angle offset applied to published servo value
    time = rospy.Time.now()
...
def read_end_servo():
    ...
    rotation = tf.transformations.quaternion_from_euler(1.570796, 0, 1) #published by end servo
    time = rospy.Time.now() 
